...
>
  <div
    use:motion
    aria-hidden={!active}
    class="bg-abyss/70 pointer-events-none fixed inset-0 z-40 flex items-center justify-center p-6 backdrop-blur-[2px]"
  >
    <div
...
{#if !hasImages}
  <Motion let:motion {...blink}>
    <div
      use:motion
      class="pointer-events-none absolute inset-x-0 bottom-16 z-20 flex flex-col items-center gap-2 text-center"
    >
      <p class="font-headers text-4xl tracking-widest text-white">[ FEED ME ]</p>
      <p class="mt-8 font-body text-xs tracking-widest text-white">
